...
app.config['SQLALCHEMY_ECHO'] = True  # DB処理時にSQLをコンソールに表示する
...
class Employee(db.Model):
    ...
    # 外部テーブルと紐付けるためのオブジェクト
    # employeesを取得すると紐付けたテーブルも取得される
    # One to Many
    projects = db.relationship('Project', backref='employees', lazy='dynamic')
    # backref: Projectからデータを取得した場合のキー
    # lazy: テーブルを紐づける場合の処理方式(default=select)
