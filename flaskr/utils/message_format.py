...
def make_message_format(user, messages):
    ...
    for message in messages:
        ...
        if user.picture_path:
            message_tag += f'<img src={url_for("static", filename=user.picture_path)} class="user-img-mini">'
        message_tag += f'''
                <p>{user.username}</p>
            </div>
...
def make_old_message_format(user, messages):
    """oldメッセージ内容をhtmlに合わせて整形"""
    message_tag = ''
    for message in messages[::-1]:  # 逆順に読み込み
        if message.from_user_id == int(current_user.get_id()):
            ...
            if current_user.picture_path:
                message_tag += f'<img src="{url_for("static", filename=current_user.picture_path)}" class="user-img-mini">'
